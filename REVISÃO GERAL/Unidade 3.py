...
# CRUD é o acronimo para as quatro operacoes DML possiveis em uma tabela de um banco de dados. CREATE = inserir, READ = ler, UPDATE
...
# RESOLUCAO
# Primeiro sera criada a classe DDL para criar o banco, criar e apagar a tabela. Entao sera criada a classe DML para as operacoes CRUD.
# Na classe 'DDLSQLite' sera criado um METODO privado que retorna uma instancia de conexao com um banco PARAMETRIZAVEL.
# Haverao 3 metodos publicos, um para criar um banco de dados, outro para criar a tabela e outro para apagar uma tabela. Todos os metodos sao parametrizaveis.
# O metodo 'criar_banco_de_dados()' cria um banco fazendo uma conexao e em seguida fecha.
...
# Todas as operacoes CRUD precisam receber como parametro o nome do banco de dados, entao foi passado o parametro no metodo construtor. Para instanciar essa classe, deve ser informado o nome do banco e apartir do objeto, chamar os metodos e passar os demais parametros.

# O nome do banco sera usado pelo metodo privado '_conectar()' que retorna uma instancia de conexao, e sera chamado internamente pelos demais metodos. O metodo 'inserir_registros' extrai como uma tupla as colunas e valores a serem usados na construcao do comando de insercao.
# O metodo 'ler_registros()' seleciona todos os dados de uma tabela e os retorna (o retorno sera uma lista de tuplas). O metodo 'atualizar_registro()' precisa dos dados a serem atualizados (dicionario) e dos dados que serao usados para construir a condicao, sendo que a atualizacao so pode ser feita se o valor da condicao for inteiro.
# O metodo 'apagar_registro' tambem so pode ser executado se o valor da condicao for inteiro.
...
#inserir registros e consultar para checar se deu certo
...
# para cada dicionario na lista de dados, invocar um metodo de insercao
for valor in dados:
